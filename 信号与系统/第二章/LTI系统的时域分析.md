...
* 在研究阶跃响应和沖激响应时，发现沖激响应本身就可以刻画整个系统的结构。只需要用激励函数与某系统的**零状态沖激响应**进行一次卷积即可得到该系统对这一激励的相应（注意这样求得的激励的响应也是零状态的，故还需求一个零输入响应才能得到全响应）。
* 此时，不再需要繁琐求解微分方程了（一般求个沖激响应就行）。
...
[高等数学的解法](../../高等数学（下）/第九章/二阶线性常系数微分方程.md)大部分仍然是适用的，它相当程序化。对于任意激励，就是
（1）先求齐次方程通解（特征方程）。
（2）再求非齐次方程特解（查表）。
（3）然后将特解代入微分方程求特解的系数。
（4）接着球系统的「初始状态」或边界条件。
（5）最后将系统「初始状态」代入完全解求通解系数。

信号与系统中微分方程与高数差别最大的地方就在于「初始状态」或者说边界条件上，**此处的起始点是可能存在跳变的**。

> 注：这些步骤的任何一步都很容易出错，得搞清楚。不要忘记在求得特解后的步骤都要用完全解。
...
（1）求 $0_- < t < 0_+$ 区间的系统响应，用奇异函数平衡待定系数解。
（2）响应及其各阶导数中含有 $u(t)$ 的项会在起始点发生跃变，对微分方程两侧同时在 $0_- < t < 0_+$ 上积分即得答案。
...
思路为，激励函数中 $\delta(t)$ 的最高阶导数应该在响应函数的最高阶导数中，然后根据奇异函数系数平衡假设相应函数的最高阶导数表达式（待定系数），列写各阶导数表达式并对等式两端在 $0_-$ 到 $0_+$ 积分求得起始值 $y(0_+)$。

这里必须注意一点，**含有沖激的激励可能使得起始点的响应的解析式跟之后的解析式不同**，即响应是在 $0_-< t < 0_+$，$t > 0$ 两段上的分段函数。求起始点跳变时若要假设响应的解析式，只用根据奇异函数平衡假设，其它任何函数，包括阶跃函数的系数都不用匹配。只要用起始点的响应解析式得知 $y(0_-)$ 和 $y(0_+)$ 的关系，就可以方便地求出初始状态。

如：若 $y''(t)$ 含有 $\delta''(t)$，则假设在 $0_- < t < 0_+$ 时有

$$
\begin{aligned}
&y''(t) = a\delta''(t) + b\delta'(t) + c\delta(t) + du(t)\\
&y'(t) = a\delta'(t) + b\delta(t) + cu(t)\\
&y(t) = a\delta(t) + bu(t)
\end{aligned}
$$
...
> 注：（1）如果要求阶跃响应 $g(t)$，可以直接利用 LTI 的微分性质 $h(t) = \dfrac{d}{dt}g(t)$ 从沖激响应求。
...
> 注：
> （1）通解的函数特性仅依赖于系统本身，与激励信号的形式无关，因而称为自由响应。但是它的系数跟激励信号有关。特解的形式只与激励信号有关。
> （2）只有零状态响应的部分能用微分性质、用沖激函数卷积直接求响应。用卷积法求全响应，零输入和零状态响应必须分开求。在一些题目中将全响应分解来应用微分性质也是有用的。
...
解的形式有了，接下来就是求系数，也是奇异函数匹配就可以了。也就是先设起始点响应求边界条件，再用奇异函数平衡求奇异函数系数，最后代边界条件求通解系数即可（对于沖激响应，通解就是完全解）。

> 注：
> （1）关于 $n <= m$ 时响应的形式的问题，你或许可以把那堆 $\delta^{(k)}(t)$ 当作特解，但它其实是一个对系统 $t > 0$ 时没有影响的玩意，是纯纯的 $0$。它纯粹是为了数学上自洽（奇异函数平衡）存在的。
> （2）激励为沖激时，特解就是 $0$，激励为阶跃时，特解就是一未知常数。

* 这是一个求沖激响应的例子。

![Screenshot-from-2022-10-22-22-19-12.png](http://image.tjzfile.xyz/images/2022/10/22/Screenshot-from-2022-10-22-22-19-12.png)
![Screenshot-from-2022-10-22-22-19-19.png](http://image.tjzfile.xyz/images/2022/10/22/Screenshot-from-2022-10-22-22-19-19.png)
![Screenshot-from-2022-10-22-22-19-34.png](http://image.tjzfile.xyz/images/2022/10/22/Screenshot-from-2022-10-22-22-19-34.png)

* 有趣的是，这个求解过程甚至还能简化，这需要用到 LTI 系统的线性和微分性质，当我们想求一个复杂沖激激励的响应时，求一个 $\delta(t)$ 的响应再线性组合就行。

![Screenshot-from-2022-10-22-22-34-09.png](http://image.tjzfile.xyz/images/2022/10/22/Screenshot-from-2022-10-22-22-34-09.png)

把这个性质（线性性质）更进一步地推广，能得到更惊人的结论。

### 卷积求任意时刻系统状态

* 重量级结论，务必参透。

上面尝试着把激励拆解成了单位沖激信号的线性组合，这里我们不妨更大胆一些，在时间上继续细分，毕竟线性时不变的假设真的很强。

任意激励信号都可以分解为无穷个沖激信号的和（而每个沖激信号又是单位沖激函数的线性组合），这就可以视为 LTI 系统**在任意极短时刻都对应一个该时刻的沖激响应**（线性），这些沖激响应在时间上的累加（时不变性）就是系统对激励信号的响应。

是不是看上去感觉很熟悉？这个过程可以形象地描述为“对一组输入信号的任意时刻都做某种变换（得到无穷个沖激响应）再累加起来得到结果”，这玩意不就是卷积吗？

所以 LTI 的零状态响应就是激励和沖激响应的卷积积分：

$$
y_{zs}(t) = \int_{-\infty}^\infty x(\tau)h(t - \tau)d\tau = f(t) * h(t)
$$

从某种意义上说，$h(t)$ 已经包含了其所对应的系统的所有时域信息了。

* 现在，你再也不用嗯求微分方程了（沖激响应只用奇异函数平衡和待定系数，最终卷积一次就完事了）。

但是必须注意，用卷积只能通过零状态的沖激响应**求激励函数的零状态响应**，所以还要额外求一个零输入响应。

![Screenshot-from-2022-10-22-22-56-26.png](http://image.tjzfile.xyz/images/2022/10/22/Screenshot-from-2022-10-22-22-56-26.png)