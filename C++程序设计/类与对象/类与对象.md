...
> 这篇笔记是很久很久以前（还没上带学）写的，所以看起来有些不可名状。
...
​	成员函数可以定义在类定义内部，或者单独使用**作用域解析运算符 ::** 来定义。
...
> 注意：友元关系是单向的，这意味着一个类的友元类可以访问该类的 $private/protected$ 成员，但是该类不能访问友元类的 $private/protected$ 成员。
...
​		2、静态成员函数**只能访问静态成员**数据、其他静态成员函数和类外部的其他函数。
...
> 注：
> ​1、继承类型一般使用 $public$，其父类 $public$ 成员在子类也是 $public$ ， $protected$ 在子类是 $private$，父类 $private$ 对子类不可见。
> 2、继承类型为 $private$，其父类的 $protected~public$ 在子类中都为 $private$ 成员，$private$ 对子类不可见。
> 3、继承类型为 $protected$，其父类的 $protected~public$ 在子类中都为 $protected$ 成员，$private$ 对子类不可见。
...
​	下面给仨例子，分别是不同的子类构造函数实现方式：

第一种：直接传入一个基类的对象调用基类复制构造函数。
...
public:
	double weight;
	ALIENWARE(double wgt, double prc);
	// 子类拷贝构造函数调用父类拷贝构造函数初始化
	ALIENWARE(const ALIENWARE &pc) : PC(pc), wight(pc.weight) {}
};
...
// 这个B就是“含有类对象作为数据成员的类”
// 直接在 B 的构造函数调用 A 的构造函数构造 a 就行
```

#### 子类的用途

子类的派生，就是一个抽象到具体的过程，一般一个类越派生功能越多，同时由于类继承的特性，代码不必重写，这对代码维护和开发是很关键的。

通常，子类有三个用途：

（1）吸收基类成员：子类继承了基类的方法和成员，在子类中可以直接调用基类的部分方法和成员。
（2）改造基类成员：如果子类声明了一个和基类某成员同名的新成员，派生的新成员就隐藏了上层同名成员，这被称作同名隐藏。
（3）添加新成员：可以根据实际需要，给派生类添加适当的数据和函数成员。
...
但是一般来说不推荐这样做，因为这通常会导致项目继承关系混乱，也会存在大量同名隐藏问题。通常继承关系使用树状结构较好。

解决我们不希望的同名隐藏可以使用作用域解析运算符 `::`，通过指定 `Class::member` 来访问特定基类的成员，如果不加，那么当子类与父类的数据或方法及其列表完全一致时，（在调用时）子类的方法实现将覆盖父类的。

> 注：可以通过 `using` 关键字消除二义性，即多继承时子类的多个父类具有同名成员的情况。具体做法是在类的最开始添加 `using BaseClass::member` 这样的语句，之后在使用这个 `member` 时，就不用加 `::`。

![mmexport1668694438359.jpg](http://image.tjzfile.xyz/images/2022/11/17/mmexport1668694438359.jpg)

但是当我们考虑上图这样的继承关系，就会导致一个小问题—— `Base0` 中的函数 `fun0()` 和数据成员 `var0` 在 `Base1` 和 `Base2` 各继承一份，此时 `Derived` 再继承，就会继承两个 `var0` 和两个 `fun0()`，虽然他们完全可以通过 `::` 进行区分（注意，不能省略 `::`，必须对不同直接基类进行区分，如 `Base1::var0`），但是这会导致额外的内存开销（同一成员在内存中有多个副本）。

**虚基类**解决了这个问题，只要在继承时在类访问修饰符前添加 `virtual` 关键字即可实现这样的功能，即顶层基类通过不同路径继承下来的成员在内存中只保存一次，这样访问 `var0` 和 `fun0()` 就不需要 `::` 了。
...
> 注：由于多继承很少使用，所以不把虚基类单独作为一个小节。

#### 类型兼容

* 任何一个需要基类的占位符，都可以传入它的子类进行代替。

（1）子类的对象可以隐式转换为基类对象，并用子类从基类继承的成员（的当前值）初始化基类对象。
...
​	2、继承关系必须有同名虚函数（**其中虚函数是在父类中使用关键字`virtual`声明的函数，在子类中重新定义父类中定义的虚函数时，会告诉编译器不要静态链接到该函数**）；

​	3、存在父类类型的**指针或者引用**（这个很重要，只有通过指针才能实现多态，这与C++本身的动态特性实现有关），通过该指针或引用调用虚函数；

#### 虚函数与纯虚函数

​	虚函数是在父类中用关键字`virtual`声明的函数。**在子类中重新定义父类中的虚函数后，使用特定对象类型调用该函数时，编译器会动态链接到所需的函数**。这意味着函数的调用是动态的，当程序执行到某个具体的位置，需要调用的函数才被决定。
...
​	纯虚函数的作用类似一个模板或范式，它要求继承它的子类重载它的虚方法。
...
> 注：
> 1、含有纯虚函数的类叫抽象类，**抽象类不能创建对象·定义实例**，但可以声明指向实现该抽象类的具体类的指针或引用。这是实现多态的基础——用一个**抽象类指针**指向不同的子类，就可以**通过同一套接口调用不同子类的不同方法**。
>​ 2、在有动态分配堆上内存的时候，析构函数必须是虚函数，但没有必要是纯虚的，否则子类新增的使用动态内存的成员将无法得到释放，因为析够函数在子类无法重载。
> 3、友元不是成员函数，只有成员函数才可以是虚拟的，因此友元不能是虚拟函数。但可以通过让友元函数调	用虚拟成员函数来解决友元的虚拟问题。
> 4、在虚函数的实现中，不用加 `virtual` 修饰。
...
​	上面这个只是最简单的重载 `+` 的情况，我们来看看更复杂的情况，即 `=`, `++`, `!`, `<<` 一类。
