...
> 注：在实际的 C++ 工程中，内存泄漏往往是致命而不易察觉的，因为很多时候代码会涉及到 `new` 一个类的对象而非直接实例化（这是为了使用对象的地址在函数间传递从而避免多次拷贝构造导致程序运行缓慢）， 这时对象的销毁就不是类内部能够控制的了，必须要由外部调用者控制。但是当工程比较复杂时，各种跨域通信满天飞，还很有可能不是同一个人写的，这个 `new` 出来的对象到底在哪里 `free` 就成了问题。
> 
> 不过一般来说具有良好规范的库和良好习惯的程序员会遵守 `new` 之后及时 `free` 的规则，举个例子。芝士我之前写的生成 RSA 密钥对的函数。
> ```cpp
>void Image_encrypt::GenerateRSAKey(std::string &pub_key, std::string &pri_key)
>{
>    size_t pri_len = 0, pub_len = 0;
>    char *_pub_key = nullptr;
>    char *_pri_key = nullptr;
>
>    int res = 0;
>    // RSA 库提供了内置的 new 方法
>    RSA *keypair = RSA_new();
...
>    res = RSA_generate_key_ex(keypair, RSA_SIZE, b, NULL);
>
>    if(!res) {
>        BN_free(b);
>        throw;
...
>
>    pub_key = _pub_key, pri_key = _pri_key;
>
>    // 这里一定要记得 free
>    BIO_free_all(pri), BIO_free_all(pub);
>    RSA_free(keypair);
>    free(_pri_key), free(_pub_key);
>}
>```
...
* 因为指针允许做变量的显式或隐式转换，所以会出现安全问题，一般的问题都出现在指向小对象的指针转换为指向大对象的指针时，或 void 类型指针的显式转换时。

第一种情况虽然正常人一般不会在代码里写，但是可能会由于一时疏忽出现问题。这种情况有两种可能：

（1）用基类指针指向派生类后，通过该指针访问派生类特有的成员。
（2）将基类的指针显式转换为派生类指针后，通过该指针访问派生类特有的成员。

第二种情况倒是比较常见，因此 C++ 建议我们使用智能指针或其他泛型实现方法。
...
此时再访问或操作 `b` 指向的变量就会出问题，因为指针以 `float` 类型指向了 `int` 变量。