...
用与梯度下降同样的手法分析，不难发现二阶泰勒展开把损失函数在 $\pmb\omega_0$ 的邻域近似成了一个二次函数，梯度决定了该点在这一近似函数关于最低点的偏移，而 $Hessian$ 矩阵决定了这个近似函数的形状。

与梯度下降不同的是，牛顿迭代法每次将参数改动到这个近似的二次函数的最小值点的位置（二次函数是凸函数）。
