...
$$
\dfrac{\partial L}{\partial \omega_j} = 2\sum_{i = 1}^m(\omega_0 + \omega_1x_1+\omega_2x_2+\cdots+\omega_mx_m-y_i),~j = 0,1,2\cdots,m
$$
...
> 注：实际上，在全连接神经网络中，如果将线性模型作为第一层， $\delta(z) = \dfrac{1}{1 - e^{-z}}$ 视为激活函数作为第二层，计算两个损失函数作为反向传播的依据，那么这个问题是不存在的。
...
数据集是 [Iris](https://www.kaggle.com/datasets/arshid/iris-flower-dataset)，但是经过了处理。
...
train_buf = pd.read_csv('flower_train.csv')
test_buf = pd.read_csv('flower_test.csv')

train_buf[['x1', 'x2']] = train_buf[['x1', 'x2']].replace(0, np.NAN)
for x in train_buf:
    if train_buf[x].isnull().sum():
        temp = train_buf[train_buf[x].notnull()]
        temp = temp[[x, 'type']].groupby(['type'])[[x]].mean().reset_index()
        
        train_buf.loc[(train_buf['type'] == 'Iris-setosa') & (train_buf[x].isnull()), x] = temp[x][0]
        train_buf.loc[(train_buf['type'] == 'Iris-versicolor') & (train_buf[x].isnull()), x] = temp[x][1]

train_buf['type'] = np.where(train_buf['type'] == 'Iris-versicolor', 0, 1)
train = np.array(train_buf)

eta = 1e-1
size_t = np.shape(train)[0]
max_step = 100

omega = np.zeros(3, dtype = float)

for step in range(max_step):
    omega_ex = [np.sum([point[j] * ((np.exp(np.sum(omega * point[:3]))) / \
               (1 + (np.exp(np.sum(omega * point[:3])))) - point[-1]) for point in train]) / size_t for j in range(3)]
    omega -= np.array(eta) * omega_ex
```