...
我们将用 SVM 解决的问题，与我们在感知机中看到的是一致的——给定一组输入空间上的点集 $\{(\pmb x_i, y_i)\}_{i = 1}^n, \pmb x_i\in R^d, y_i\in \{+1, -1\}$，希望找到一个分类超平面 $f(x) = \text{sgn}(\pmb\omega^T\pmb x + b)$，将 $y = +1$ 的点和 $y = -1$ 的点正确分开。
...
有趣的是，这个问题仍然是一个凸二次规划问题，我们便可以尝试直接通过对 $\pmb\alpha$ 求出它的解 $\alpha^*$，进一步回代（还需要利用 KKT 条件来求，如果忘记可以再看一下[上一节](./SVM及其数学基础.md)内容）就能得到 $\pmb\omega^*, b^*$ 了，而这正是最优超平面的参数。下面假设我们已经求得 $\alpha^*$，来回代求解 $\pmb\omega^*, b^*$。
